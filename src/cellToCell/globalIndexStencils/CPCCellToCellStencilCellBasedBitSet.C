...
void Foam::CPCCellToCellStencilCellBasedBitSet::calcPointBoundaryData
(
...
{
    addProfiling(stencil, "CPCCellToCellStencilCellBasedBitSet::calcPointBoundaryData");
...
void Foam::CPCCellToCellStencilCellBasedBitSet::calcCellStencil
(
    labelListList& globalCellCells
) const
{
    addProfiling(stencil, "CPCCellToCellStencilCellBasedBitSet::calcCellStencil");
    ...
    // bitSet cells(globalNumbering().offsets()[globalNumbering().offsets().size()-1]);
    ...
    forAll(globalCellCells,celli)
    {
        ...
        label minLabel = min(neiCells);
        label maxLabel = max(neiCells);

        bitSet cells(maxLabel-minLabel,false);


        // cells.reset();
        cells.set(neiCells);
        neiCells.clear();
        for (const label nei : cells)
        {
            neiCells.append(nei);
        }
...
Foam::CPCCellToCellStencilCellBasedBitSet::CPCCellToCellStencilCellBasedBitSet(const polyMesh& mesh)
:
